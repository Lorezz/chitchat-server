...
const sendMail = async (email, code) => {
  ...
  const ctaUrl = `${frontendUrl}/verify?email=${email}&code=${code}`;
  // const qrcodeImg = await qrcode.getDataUrl(ctaUrl);
  const qrcodeImg = `${process.env.HOST}/qr?text=${encodeURI(ctaUrl)}`;
  log.wtf(qrcodeImg);
  const params = {
    ...
    cta_message:
      'Click the button to directly go to verification from your default browser the code',
    cta_button: 'Magic link',
